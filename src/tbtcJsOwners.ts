...
import {TokenStaking, TokenGrant, provider, StakingPortBacker, TokenStakingEscrow} from './contracts'
...
export function lookupOwner(operator:string, blockTag:number) {
  ...
  async function resolveOwner(owner:string, operator:string, blockTag:number):Promise<string> {
    if ((await provider.getStorageAt(owner, 0)) === "0x") {
      ...
      const { owner } = await StakingPortBacker.copiedStakes(operator, {blockTag})
      return resolveOwner(owner, operator, blockTag)
    } else if (owner == TokenStakingEscrow.address) {
        const [{
            args
          }] = await TokenStakingEscrow.queryFilter(
  ...
  const TokenGrantStakedABI = {
    ...
    constant: undefined,
    payable: undefined,
    signature: '0xf05c07b89b3e4ff57b17aa6883ec35e90d7710c57a038c49c3ec3911a80c2445'
  }
  ...
  async function resolveGrantee(
    ...
    if ((await provider.getStorageAt(grantee, 0)) === "0x") {
      ...
      try {
        const grant = new ethers.Contract(
         grantee,
          ManagedGrantABI,
          provider
        );
